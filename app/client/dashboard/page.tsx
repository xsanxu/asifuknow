  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const upcomingEvents = allEvents.filter(
        (e) => new Date(e.shift_date) >= new Date() && e.status === 'active'
      );
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-2xl font-bold">{(profile?.rating_avg ?? 0).toFixed(1)} / 5.0</span>
              </div>
              <p className="text-primary-100">
                Your Rating ({profile?.rating_count ?? 0} reviews)
              </p>
            </div>
            <div className="text-right">
              <div className="text-2xl font-bold mb-1">{profile?.payment_score ?? 0}%</div>
